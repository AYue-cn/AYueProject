...
BACKUP_FILE = os.path.join("cache", "sora_characters_backup.json")  # 新增备份文件
...
class SoraCharacterUploader(ctk.CTk):
    def __init__(self):
        ...
        self.downloader = GiteeFolderDownloader(
            ...
            app=self  # 传入 app 实例，以便在 downloader 中调用 log
        )
    ...
    def _merge_characters(self) -> List[Dict]:
        """合并本地备份和云端下载的角色（按character_id去重，保留所有记录）"""
        # 1. 读取本地备份
        local_backup = []
        ...
        for item in cloud_data:
            ...
            if cid:
                merged[cid] = item
        # 再加本地备份的（覆盖/新增，保留本地最新）
        for item in local_backup:
            cid = item.get("character_id")
            if cid:
                merged[cid] = item
            else:
                # 无character_id的也保留（防止异常数据丢失）
                merged[str(hash(json.dumps(item, sort_keys=True)))] = item
    ...
    def create_widgets(self):
        ...
        self.download_btn = ctk.CTkButton(
            btn_frame, text="下载角色", width=220, height=40,
            command=self.start_download_thread
        )
        self.download_btn.pack(side="left", padx=10)

        self.upload_cloud_btn = ctk.CTkButton(
            btn_frame, text="上传合并角色", width=220, height=40,
    ...
    def log(self, message, error=False):
        ...
        color_tag = "error" if error else "success" if any(
            word in message for word in ["成功", "完成", "已生成", "保存"]) else "info"
    ...
    def process_and_upload(self, input_path, timestamps):
        ...
        try:
            ...
            if character_id:
                ...
                self.save_characters()  # 保存到本地（这一步是新增角色的关键）
    ...
    def start_download_thread(self):
        ...
        self.status_label.configure(text="下载进行中...", text_color="yellow")

        threading.Thread(target=self.download_and_merge, kwargs={"is_upload_prepare": False}, daemon=True).start()

    def download_and_merge(self, is_upload_prepare: bool = True):
        """
        核心下载逻辑
        :param is_upload_prepare: 是否是上传前置步骤（True=需要备份+合并，False=单独下载直接覆盖）
        """
        try:
            self.log("=== 开始执行下载逻辑 ===")

            # 上传前置步骤：先备份本地角色文件
            if is_upload_prepare:
                self._backup_local_characters()

            # 执行原有的下载逻辑
            self.log(f"正在从 Gitee 下载 {self.remote_folder} 文件夹到本地...")
            ...
            self.log(f"下载完成！本地已同步云端内容，当前角色数：{len(self.characters)}")

        except Exception as e:
            self.log(f"下载发生错误：{str(e)}", error=True)

        finally:
            # 仅在单独下载时恢复按钮状态
            if not is_upload_prepare:
                self.task_running = False
                self.after(0, lambda: self.download_btn.configure(state="normal", text="下载角色"))
                self.after(0, lambda: self.status_label.configure(text="就绪", text_color="gray"))

    def start_upload_cloud_thread(self):
        """点击上传合并角色：先备份→下载→合并→上传"""
        if self.task_running:
            messagebox.showwarning("提示", "已有任务正在进行，请等待完成")
            return
    ...
    def upload_to_cloud(self):
        ...
        try:
            ...
            self.download_and_merge(is_upload_prepare=True)  # 标记为上传前置，执行备份+合并

            # 第三步：执行上传逻辑
            self.log("=== 下载+合并完成，开始执行上传逻辑 ===")
            self.log("开始分类型上传本地文件夹到 Gitee（图片POST/JSON PUT）...")
            self.downloader.upload_folder(self.cache_dir, self.remote_folder)
            self.log("=== 上传合并完成！本地+云端角色已合并并上传 ===")
            ...
            self.task_running = False
            self.after(0, lambda: self.upload_cloud_btn.configure(state="normal", text="上传合并角色"))
            self.after(0, lambda: self.status_label.configure(text="就绪", text_color="gray"))
...
class GiteeFolderDownloader:
    ...
    def log(self, message, error=False):
        """如果有 app 实例，则输出到 GUI 日志，否则输出到控制台"""
        if self.app:
    ...
    def upload_folder(self, local_folder: str, remote_folder: str):
        """
        分类型上传：
        - 图片文件（.jpg/.png/.jpeg）：POST + formData
        - JSON文件（.json）：PUT + JSON（带sha）
        """
        self.log(f"开始分类型上传本地文件夹 {local_folder} 到 Gitee {remote_folder}...")

        # 1. 遍历本地文件
        local_files = []
        image_ext = [".jpg", ".png", ".jpeg", ".gif", ".bmp"]  # 缩略图文件类型
        json_ext = [".json"]  # JSON文件类型
