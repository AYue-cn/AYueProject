...
"""
    ...
    def _init_ai_client(self):
        ...
        try:
            ...
            if not api_key:
                # 尝试从代码中设置（实际使用时建议通过环境变量设置）
                os.environ.setdefault("ARK_API_KEY", "d93cef80-19e9-4643-888d-05d4da4ec2c1")
    ...
    def _create_widgets(self):
        ...
        footer_label = ttk.Label(
            self.root,
            text="说明：文件分割后可点击AI处理按钮提交给豆包AI，结果将保存为新文件",
            font=("微软雅黑", 8),
            foreground="#666666"
        )
    ...
    def process_with_ai(self, file_path):
        ...
        if not self.ai_client:
            messagebox.showerror("AI客户端错误", "豆包AI客户端初始化失败，无法进行AI处理")
            return
        ...
        self._update_log(f"📤 正在向豆包AI提交文件：{os.path.basename(file_path)}...\n")
        ...
        def ai_process_thread():
            try:
                ...
                self._update_log(f"✅ AI处理完成，结果已保存至：{os.path.basename(ai_result_path)}\n")
    ...
    def split_txt_by_chinese(self, source_path, split_count=5000):
        """按指定汉字数分割txt文件，自动尝试编码，确保以句号结尾并附加修改要求"""
        try:
            # 解析源文件路径和名称
