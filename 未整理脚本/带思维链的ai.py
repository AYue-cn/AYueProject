...
"""
    ...
    def _create_widgets(self):
        ...
        ttk.Label(config_frame, text="（豆包AI的API Key）", font=("微软雅黑", 8)).grid(
        ...
        ).grid(row=1, column=3, padx=20, pady=5, sticky="w")
        ...
        self.log_text.insert(tk.END, "请先输入API密钥，再选择要分割的TXT文件...\n")
        ...
        footer_label = ttk.Label(
            self.root,
            text="说明：每次启动需手动输入API密钥，所有输出文件均以UTF-8编码保存",
            font=("微软雅黑", 8),
            foreground="#666666"
        )
    ...
    def select_files(self):
        """选择多个TXT文件并处理（先验证API密钥）"""
        # 验证API密钥
        api_key = self.api_key_var.get().strip()
        if not api_key:
            messagebox.showwarning("缺少API密钥", "请先输入有效的豆包AI API密钥")
            return

        # 初始化AI客户端（如果尚未初始化或密钥已变更）
        self.ai_client = self._init_ai_client()
        if not self.ai_client:
            messagebox.showerror("AI客户端初始化失败", "无法使用提供的API密钥初始化AI客户端")
            return
    ...
    def process_with_ai(self, file_path):
        ...
        if not self.ai_client:
            self.ai_client = self._init_ai_client()
            if not self.ai_client:
                messagebox.showerror("AI客户端错误", "无法初始化AI客户端，请检查API密钥")
                return
        ...
        file_name = os.path.basename(file_path)
        self._update_log(f"📤 正在向豆包AI提交文件：{file_name}...\n")
        ...
        def ai_process_thread():
            try:
                ...
                self._update_log(f"🤖 豆包AI正在处理，请稍候...\n")
                ...
                self._update_log(f"✅ AI处理完成，结果已保存至：{os.path.basename(ai_result_path)}（UTF-8编码）\n")

                # 更新文件列表
                self.processed_files.append(ai_result_path)
                self.root.after(0, self._update_file_buttons)

            except ArkException as e:
                error_msg = f"❌ 豆包AI接口错误：{str(e)}\n"
                if "API key" in str(e):
                    error_msg += "请检查您的API Key是否有效\n"
