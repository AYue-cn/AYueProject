...
class VideoDuetApp(ctk.CTk):
    def __init__(self):
        ...
        # 音频来源变量（关键：提前定义，避免AttributeError）
        ...
        self.mode_seg = ctk.CTkSegmentedButton(
            self, values=["随机模式", "穷举模式", "1vN模式"],  # 新增1vN模式
        ...
        # 生成数量（1vN模式下N值）
        ctk.CTkLabel(self, text="生成数量（随机模式/N值）：").grid(  # 修改标签提示
            row=row, column=0, padx=pad_x, pady=pad_y, sticky="w"
        )
        ...
        self.btn_cancel.pack(side="left", padx=pad_x, pady=10)
        ...
        self.btn_open_output.pack(side="left", padx=pad_x, pady=10)
        row += 1
    ...
    def show_about(self):
        ...
        msgbox.showinfo(
            "关于",
            "阿岳视频拼接助手v3.0\n\n作者：阿岳\nEmail：zyclovewyc@gmail.com\n版本：3.0\n功能：左右渐变蒙板拼接短视频\n支持随机模式/穷举模式/1vN模式，音频/时长选择、渐变宽度调节等\n\n感谢使用！"
        )
    ...
    def on_mode_change(self, *args):
        """模式切换回调：仅穷举模式禁用生成数量输入框"""
        if self.mode_var.get() == "穷举模式":
            self.entry_num.configure(state="disabled")  # 穷举模式禁用
        else:
            self.entry_num.configure(state="normal")  # 随机/1vN模式启用
    ...
    def start_generation(self):
        ...
        if count_a == 0:
            ...
            self.log("错误：A文件夹中未找到视频文件！\n")
            return
        if count_b == 0:
            msgbox.showerror("错误", "B文件夹中未找到视频文件！")
            self.log("错误：B文件夹中未找到视频文件！\n")
            return
        ...
        if self.mode_var.get() in ["随机模式", "1vN模式"]:
            ...
            if self.mode_var.get() == "1vN模式":
                if count_b < n_value:
                    ...
                    self.log(f"错误：1vN模式下，B文件夹视频数量({count_b}个) < N值({n_value})，无法生成！\n")
                    return
    ...
    def generate_videos(self):
        ...
        try:
            ...
            if self.mode_var.get() == "随机模式":
                ...
                total = len(pairs)
                self.log(f"穷举模式：将生成 {total} 个视频（{count_a} × {count_b}）\n")

            elif self.mode_var.get() == "1vN模式":
                # 新增1vN模式：每个A视频随机匹配N个不重复的B视频
                n_value = int(self.num_generate.get())
                total = count_a * n_value  # 总生成数量=A数量×N
                self.log(f"1vN模式：{count_a}个A视频 × {n_value}个B视频 = 共生成{total}个视频\n")

                # 遍历每个A视频，随机选N个B视频（不重复）
                for a_file in videos_a:
                    # random.sample：从B列表中随机选n_value个不重复的元素
            ...
            if not self.is_cancelled:
                self.log(f"{self.mode_var.get()}：全部{total}个视频生成完成！\n")
